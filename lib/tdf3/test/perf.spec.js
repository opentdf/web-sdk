...
async function correctnessTrial(
  plainFile,
  sourceType,
  sourceSize,
  trialType = 'correctness',
  pipeType = 'file'
) {
  ...
  async function pipeTest() {
    ...
    switch (pipeType) {
      case 'buffer':
        source = { type: 'buffer', location: await encryptedStream.toBuffer() };
        break;
      case 'file':
        await encryptedStream.toFile(`${baseDir}/teststream-${sourceSize}.zip`);
      ...
      case 'stream':
        ...
        break;
      default:
        throw new Error('unrecognized pipeType', pipeType);
    }
    ...
    await decryptStream.toFile(`${baseDir}/teststream-${sourceSize}.out`);
    ...
    describe(`I/O Type: ${streamType}`, function () {
      for (const [name, sizeInBytes] of Object.entries(sizes)) {
        it(`Round ${name}`, async function () {
          ...
          results.push(result);
          console.log(
            `${streamType}:${name} Total Duration: ${ms}, Normalized: ${result.normalizedScore}, EncryptTime: ${result.encryptTimeMs}, DecryptTime: ${result.decryptTimeMs}`
          );
        });
      }
    });
...
describe('Local Correctness Tests', function () {
  for (const pipeType of ['buffer', 'file', 'stream']) {
    for (const streamType of ['file-node', 'file-stream']) {
      it(`Round trip ${streamType} ${pipeType}`, async function () {
        const { path, size } = plainFiles['1-KiB'];
        // TODO Update correctness trial to take crypto types
        await correctnessTrial(path, streamType, size, 'correctness', pipeType);
      });
    }
  }
});
