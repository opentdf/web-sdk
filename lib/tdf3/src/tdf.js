...
class TDF extends EventEmitter {
  ...
  async writeStream(byteLimit, isRcaSource) {
    ...
    */
    ...
    await this.contentStream.pipeThrough(transformStream).pipeTo(plaintextStream.writable);
  ...
  async readStream(chunker, outputStream, rcaParams) {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      writer.write(decryptedSegment.payload.asBuffer());
    }
