...
class TDF extends EventEmitter {
  ...
  addContentStream(contentStream, mimeType) {
    this.contentStream = (contentStream instanceof ReadableStream)
      ? contentStream
      : toWebReadableStream(contentStream);
    this.mimeType = mimeType;
    return this;
  }
  ...
  async writeStream(byteLimit, isRcaSource) {
    ...
    */
    const transfromer = {
      ...
      async flush(controller) {
        try {
          ...
          controller.enqueue(zipWriter.writeDataDescriptor(crcCounter, fileByteCount));
          _countChunk(zipWriter.writeDataDescriptor(crcCounter, fileByteCount));
    ...
    const transformStream = new TransformStream(transfromer, { highWaterMark: segmentSizeDefault }, { highWaterMark: segmentSizeDefault });
    const plaintextStream = new PlaintextStream(segmentSizeDefault);

    console.log('pipe started');
    const transformed = await this.contentStream.pipeThrough(transformStream);
    console.log('transformed:');
    console.log(transformed);
    await transformed.pipeTo(plaintextStream.writable);
  ...
  //       try {
  //         outputStream.getWriter().write(chunk);
