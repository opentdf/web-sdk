 ...
 * dropped connection — precisely the distinction a caller needs to make, since
 * one is an attack and the other is worth retrying. A reader rejects with the
 * error the stream was errored with, so `IntegrityError` survives the trip.
 */
