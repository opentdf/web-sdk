...
import DecoratedTransformStream from './DecoratedTransformStream';

class BrowserTdfStream extends DecoratedTransformStream {
  /**
   * Dump the stream content to a string. This will consume the stream.
   * @return {string} - the plaintext in string form.
   */
  async toString() {
    return await new Response(this.readable).text();
  }
   ...
   */
  async toBuffer() {
    return Buffer.from(await new Response(this.readable).arrayBuffer())
  }
  ...
  async toFile(filepath, encoding) {
    ...
    if (WritableStream && this.readable.pipeTo) {
