...
import mixin from 'streamMixin'; // alias from webpack

/**
 * {@link https://nodejs.org/api/webstreams.html}
 * Reason why its used cause we want to make Web-first cross browser app that
 * locker-compliant (https://developer.salesforce.com/docs/component-library/documentation/en/lwc/js_third_party_guidance)
 * which means we cant use polyfills. Luckily node has implementation of web-streams.
 * Difference only in converting stream. Web uses new Response(stream) that can be converted with anything
 * and node uses https://nodejs.org/api/webstreams.html#streamconsumersarraybufferstream
 *
 ...
 * @hideconstructor
 */
export class PlaintextStream extends ReadableStream { // ReadableStream doesnt have event system build in
  constructor(byteLimit) {
    super({}, { highWaterMark: byteLimit });
    Object.assign(this, new EventEmitter(), mixin)
  }
