/**
 * `toBuffer()` must not launder a decrypt failure into a network error.
 *
 ...
 * a dropped connection — and the first is an attack while the second is worth
 * retrying.
 *
 * These run under both mocha (Node) and karma (Chrome); Node preserves the
 * original error either way, so the browser run is the one that matters.
 */
