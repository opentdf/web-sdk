...
describe('ML-KEM crypto', () => {
  for (const level of [768, 1024] as const) {
    describe(`ML-KEM-${level}`, () => {
      it('generateMlKemKeyPair produces correctly-sized keys', async () => {
        ...
        const expectedPkSizes = { 768: 1184, 1024: 1568 };
        const expectedSkSizes = { 768: 2400, 1024: 3168 };
      ...
      it('SPKI DER round-trip preserves raw key bytes', () => {
        const expectedRawSize = { 768: 1184, 1024: 1568 }[level];
      ...
      it('rejects a BIT STRING that over-claims past the DER buffer', () => {
        const expectedRawSize = { 768: 1184, 1024: 1568 }[level];
        const raw = new Uint8Array(expectedRawSize).map((_, i) => i & 0xff);
        const der = encodeMlKemSpkiDer(raw, level);
        // BIT STRING content = rawKey + 1 unused-bits byte, encoded with a 2-byte
        // long-form length header immediately before it. Inflate that header in
        // place: the outer SEQUENCE length still matches der.length, but the BIT
        // STRING now claims more bytes than remain. Without the bounds check this
        // slid through slice() as a silently-truncated rawKey.
        const contentStart = der.length - (expectedRawSize + 1);
        der[contentStart - 2] = 0xff;
        der[contentStart - 1] = 0xff;
        expect(() => decodeMlKemSpkiDer(der)).to.throw(/BIT STRING length does not match/);
      });
  ...
  describe('isPublicKeyAlgorithm', () => {
    it('returns true for all 7 valid tokens', () => {
      const valid = [
        'ec:secp256r1',
        'ec:secp384r1',
        'ec:secp521r1',
    ...
    it('returns false for invalid tokens', () => {
      const invalid = ['ec:foo', 'rsa:1024', 'mlkem:256', 'mlkem:512', 'aes:256', '', 'rsa'];
