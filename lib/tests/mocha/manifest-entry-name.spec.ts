 ...
 *
 * The spec says `manifest.json` at the archive root; this SDK has always written
 * and read `0.manifest.json`. The `0.` prefix is a holdover from an early design
 * that allowed several payload/manifest pairs per archive and never shipped.
 *
 * The writer now emits the spec name. The reader accepts either, preferring the
 * spec name, so archives written by earlier releases keep opening. `0.payload` is
 * untouched: it is recorded in the manifest's `payload.url`, so renaming it would
 * change manifest contents rather than just archive layout.
 */
...
async function renameManifestEntry(buffer: Uint8Array, newName: string): Promise<Uint8Array> {
  ...
  for (const cd of await centralDirectoryOf(buffer)) {
    const fileName = cd.fileName === manifestFileName ? newName : cd.fileName;
...
describe('manifest entry name (platform#3513)', function () {
  describe('manifestEntryName', function () {
    ...
    it('asks for the spec name when the archive carries neither', function () {
      assert.equal(manifestEntryName([entry('0.payload')]), manifestFileName);
    });
  });
  ...
  describe('writer', function () {
    ...
    it('names the manifest entry manifest.json', async function () {
